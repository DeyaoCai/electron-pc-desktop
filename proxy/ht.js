...
startLoad('http://www.netbian.com/', ret => {
  fs.writeFileSync("./proxy/list.json", JSON.stringify(ret));
...
function loadSubPage (list) {
  ...
  const pathes = fs.readdirSync("./img/").map(item => item.split(".")[0]);
  ...
  http.get(now.src, (res) => {
    ...
    res.setEncoding('utf8');
    let rawData = '';
    res.on('data', (chunk) => { console.log("loading subpage!") ; rawData += chunk; });
    res.on('end', () => {
      const result = [];
      try {
        const $ = cheerio.load(rawData);
...
function loadSubbPage(url,list,name){
  http.get(url, (res) => {
    ...
    res.setEncoding('utf8');
    let rawData = '';
    res.on('data', (chunk) => { console.log("loading subbpage!");rawData += chunk; });
    res.on('end', () => {
      const result=[];
      try {
        const $=cheerio.load(rawData);
...
function loadImg(url,list,name){
  http.get(url, (res) => {
    ...
    res.setEncoding('binary');
    let rawData = '';
    res.on('data', (chunk) => { console.log("loading Img!");rawData += chunk;
    });
    res.on('end', () => {
      try {
        mkdir("./img");
        fs.writeFileSync("./img/"+name+"."+url.split(".").pop(),rawData,"binary");
        setTimeout(()=>{
          console.log("load img success! now start next mission!")
          loadSubPage(list);
        },Math.random()*3000+500)
